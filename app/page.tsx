...
import { ArrowUpRight, Users, X, Hash, ArrowRight, Dice5, Flag } from 'lucide-react';
...
export default function Home() {
    ...
    useEffect(() => { const ctx = (document as any).modelContext; if (!ctx?.registerTool)
        return; const lifecycle = new AbortController(); Promise.resolve(ctx.registerTool({ name: 'filter_games_by_players', description: '按人数筛选首页游戏，0表示全部；不创建房间。', inputSchema: { type: 'object', properties: { players: { type: 'integer', enum: [0, 1, 2, 3, 4, 5, 6] } }, required: ['players'], additionalProperties: false }, execute(input: any) { if (![0, 1, 2, 3, 4, 5, 6].includes(input?.players))
            throw Error('人数必须为0或1～6'); setCount(input.players); return { games: games.filter(g => !input.players || supportsPlayers(g.id,input.players)).map(g => g.name) }; } }, { signal: lifecycle.signal })).catch(() => { }); return () => lifecycle.abort(); }, []);
    ...
    <main className="lobby"><div className="intro"><div><span className="eyebrow"><span className="dot"/> 好友到齐，随时开局</span><h1>今天，来玩点什么<span>？</span></h1><p>一个人轻松开局，也可以邀上朋友，围坐一桌。</p></div><div className="small-note"><span>PLAY TOGETHER</span><strong>好玩，在一起。</strong></div></div>
